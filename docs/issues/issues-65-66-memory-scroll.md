...
- **修复方向(未实施)**:campaign 检测连续 N 步 `applied` 停滞(仍 clamp)时主动 **bounce**——临时赋 `scrollTop=0` 再按视口步长(0→600→1200→…→savedTop)步进,视口扫过的每个区带被迫布局,最终赋值即可落地;步进值均由 campaign 自身应用,现有 `rescueApplied` 握手与接管判定天然兼容。

**对外口径**:#67 = 1e4ef0a7 修复、随下版发布;#68 = 44f3a517 修复主路径(预算+防污染,斩断"逐次劣化"级联)+ bounce 补丁封死锁,同版发布;两 issue 待补丁落地后一并回复请用户复验。
