...
维持 ds D5。**重启触发条件**（满足其一再排期）：① 独立页上线后收到 ≥3 条「希望树与列表同屏」的用户反馈；② 侧栏宽度中位数数据（可通过 announce 调研或截图分析）显示 ≥480px 成为常态。届时按 `docs/v4task-2.md:313,393` 的 panel 迁移主线实施，不在 4.1.0 嫁接半成品。
