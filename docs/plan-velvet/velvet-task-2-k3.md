...
> **提前落地记录（2026-08-26，feature/webstore-store-assets）**：`shots-store.js` 已按本节规格实施并通过 Docker 实跑（四主题瓦片 + 带右键菜单主卡 + search/recent/stats/dead/palette/选项页整页共 11 瓦片 → 按商店展示上限 5 张产出 4 张候选：`strip.png` 1400×560 四主题条带、`promo.png` 1280×800 视图拼贴（对齐手工 `vBookmarks-v4.png` 版式）、`themes.png` 1280×800 ink|paper 重点主题对开、`options.png` 1280×800 选项页真全景——宽视口 + 强制 6 栏（覆盖 `.options-page` 的 max-width:1760）把 20 组设置整页装进一框，手工参考图只敢放一半组；第 5 槽位留给品牌横幅；非扩展请求全 abort 保离线确定性；classic 主题落地后往 `STRIP_THEMES` 加一项即可）。listing 元信息同步升级：官方 V2 REST 无 listing 端点，故以 `publish.js listing`（公开页快照 + extName/extDesc 比对）与 `listing-draft`（规范源双语草稿 + 截图册规格核对）落地「取回/更新准备」，上传纪律不变。
