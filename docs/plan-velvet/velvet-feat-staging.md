...
- 选项页「暂存和最近添加」组新增 `stagingEnabled`（SYNC 键，默认开）。关闭后：暂存视图只保留最近添加列表（无工具条/分组/桶/剪刀线，徽标归零）；树/搜索/统计/死链/去重/标签组的全部暂存按钮、工具栏批量钮、选择条按钮与右键菜单项一律不渲染（各视图经 `api.isEnabled()` 门控，context-menu 在 open 末段统一压掉全部暂存条目）；树行快捷钮中的飞机位随开关消失（编辑/删除不受影响）。开关翻转会实时 adopt+重绘（含整树重建）。
- 同轮新增：树视图行尾悬浮快捷钮 `[编辑][发送到暂存][删除]`（文件夹行无飞机位），选项页「书签树」组 `treeRowActions` 开关（默认开，SYNC 键）。
