...
| Dracula 有没有比我们做得好的地方？ | **有，核实后剩九条**（§1.6 镜鉴清单，已剔除我们其实已有的四项平局）。集中在反馈丰富度与静态可扫读性；其中四条已在附录 B 立项 token，其余建议作为设计输入带进 velvet S1/S4/S6 评审。 |
...
### 1.6 镜鉴清单：Dracula 比我们做得好的地方（全局回顾）

对手值得学的地方和它的覆盖战争同样有信息量。以下逐条对照 4.1.1 HEAD 核实过，先剔除平局再记胜场——**防止把「我们其实也有」记成对方优势，是这类评审最容易自欺的地方**。

**先剔除的平局**（Dracula 做了、我们也做了的）：对话框主次按钮层级（`neat.css:1183` 已有 accent 主按钮 + 字重 600，他们的版本只是多一层渐变）；搜索命中 `mark` 的 accent 化（`neat.css:3035` 已是 accent 18% 底 + 600 字重）；搜索框焦点态（`neat.css:347` accent 边 + focus-ring 描边，vs 他们的柔光环——同量级）；语义色纪律（双方同构，我们的还是 token 驱动）。favicon 体系我们远超（反色/补全/画廊三层），不列。

**核实后成立的九条真优势**（按可吸收价值排序）：

| # | 他们做了什么 | 我们现状 | 为什么算他们赢 | 吸收路径 |
|---|---|---|---|---|
| 1 | **行的静态物件感**：每行常态即有 14% 软底 + 发丝边 + 8px 圆角 + 2/4 外距——书签行是一颗颗可指认的「对象」 | 行静态 = 透明平面（`neat.css:554-577`，无边无底），hover 才给背景 | 书签管理器里行就是主对象，静态物件感提升扫读与点击目标感；我们的行靠图标列对齐撑结构，行的边界感全靠 hover 补 | velvet §1.7 状态语言之外补「静态行材质」议题：作为材质/密度档位评审（物件感 vs 密度净损，结合 §4.8） |
| 2 | **文件夹色彩层级**：`--text-folder` 淡紫粉，文件夹与书签在颜色上即分层 | 文件夹与书签同用 `--vbm-fg`，只靠图标区分 | 深树里色相是比图标更快的一级扫描线索 | 候选 token `--vbm-fg-folder`（默认 = `--vbm-fg`，零变化），velvet S1 词表评审 |
| 3 | **hover 光条语言 + 动作列避让**：hover = 底色 + 青色下划线光条；staging 行光条右缩 30px **主动避让行尾动作列** | hover = 仅背景变化 | 光条让 hover 反馈不依赖底色差（低对比主题的刚需）；「装饰避让动作槽」这条纪律连我们部分视图都没这么细 | `--vbm-row-hover-rule`（附录 B 已立项）；避让量随 §1.9 右缘槽 token 联动 |
| 4 | **焦点/选中态的活性**：呼吸动画（`filter: brightness` 2.6s——恰是 velvet §6.2 合法的动画属性）+ hover z1/选中 z2 分层辉光 + dnd 件 z50 守护 | 选中/焦点全静态（底色 + 2px 描边） | 键盘导航的焦点行是「活的」，视线追踪成本更低；分层堆叠纪律严谨 | `--vbm-row-focus-animation`（附录 B 已立项，默认 none）；强度取 ink 磷光的克制档而非全霓虹 |
| 5 | **连接线响应 hover**：悬停子树时该子树导轨整体增强（`ul:hover` + tick 高亮）——**结构范围感知** | 2026-08-27 连接线体系完全静态 | 悬停时「点亮我所在的子树」是层级 UI 的高价值反馈，尤其深树 | velvet S6 状态语言候选创意：folder 头 hover 时其后代连接段增强（几何仍归我们，主题只给强度 token） |
| 6 | **reduced-motion 默认拒绝策略**：`*` 全局 duration 钳到 0.01ms **再叠加**逐动画 `none`——新增动画天然被兜住 | `neat.css:3849` 枚举式（每个动画记得自己登记；新动画忘登记就漏网） | 默认拒绝 vs 登记制：健壮性模式差异，不是覆盖面差异 | design-system 契约候选：全局钳制兜底 + 白名单豁免，替代逐个登记 |
| 7 | **字重作为状态信号**：基础 400，选中行/active tab/mark/主按钮统一升 500——排印参与状态语言 | 字重只有 400 与强调 600 两档（600 只给主按钮与 mark，状态行不动） | 500 状态档是「不用颜色也能读出状态」的无障碍友好分层 | velvet §1.4 排印评审：加「状态字重档」规则 |
| 8 | **滚动条可抓取性**：13px 宽 + padding-box 内嵌 + hover 渐变 | 8px（`neat.css:2998`） | 数百行的列表里粗滚动条是真实工效学（抓取目标面积 ×2）；渐变纯个性 | 密度/可抓取权衡议题，随 §4.8 一起评（注意 scrollbar 门禁矩阵联动） |
| 9 | **current 徽章降饱和批评（v4.1.1 层）**：发现我们的小 pill accent 满填过强，**本地软化且明示不动全局 accent**（原文注释写明取舍） | `.row-badge.current`/窗口 current 徽章 accent 满填，无人提出过异议 | 一条对我们设计的有效批评 + 「局部强度管理」的示范工程实践 | velvet S6 状态表复核项：小面积 accent 满填是否需要统一降档规则 |

次级观察两条（不立项但记录）：**排印微调包**——行级 `letter-spacing: 0.1px`、`line-height: 1.35em`、`-webkit-font-smoothing: antialiased`（后者仅 macOS 生效，暗底排印细节，我们 0 处）；**文件内变更日志实践**——dated 地层 + 每段「为什么」注释（§1.1 地层⑥），这套作者纪律直接进 §4.7 作者指南当范本。

**全局回顾（一段总评）**：两套设计是两种哲学的对照实验——Dracula 的世界是「**行即对象、状态会呼吸、反馈多通道**」（底色、边框、光条、辉光、动画五通道并发），我们的世界是「**克制、平面、单通道、校准几何**」（feedback 几乎只走背景一通道，换来的是密度、一致性与零几何回流）。他的赢面集中在**静态可扫读性与反馈丰富度**——恰恰是四轮用户反馈打磨出的 design-laws（右缘轴、槽位网格、状态即所得）不曾覆盖的两个维度；我们的赢面在密度、性能预算与可维护性，代价是界面「安静得近乎腼腆」。这份文件证明两者不必二选一：**token 化的主题体系本来就是让多种美学共存为选择而非裁决的机制**——光条、辉光、呼吸、hover-rule 四项已进附录 B 的 token 缺口清单，文件夹色、状态字重、连接线活性、reduced-motion 默认拒绝、行物件感五项建议作为设计输入正式带进 velvet S1/S4/S6 的评审，由设计定夺吸收与否，而不是由这份分析代答。
