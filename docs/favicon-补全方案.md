...
3. **补全来源分层**：① 直连 `https://host/favicon.ico`（主，实测可行）→ ② 页面 HTML 解析 `<link rel=icon>`（覆盖非标准路径）→ ③ DDG icons 聚合（兜底 403/反爬）。**无需第三方隐私依赖即可覆盖多数场景**。
...
| DDG `icons.duckduckgo.com/ip3/X.ico` | 可用，返回真实 ICO（与 github 一致） | 二级兜底 |
...
_favicon 返回占位图 (识别出真实 favicon 缺失)
  ...
  → [L2] fetch 页面 HTML → 解析 <link rel=icon>/apple-touch-icon/manifest (timeout 5s)
      ...
      → 全失败?
  → [L3] fetch DDG icons (可选, 需选项开关) (timeout 3s) — 覆盖 403/反爬
      → 成功? 注入 ✓
...
4. **选项开关**：`faviconEnrich`（默认开？或默认关，需评估隐私观感）——书签管理器对已知站点 fetch favicon.ico 属合理行为，DDG 作为 L3 需在选项说明。
...
|---|---|
| 大书签库首次并发 | 限流 + 队列 + 缓存 |
| 站点 403/反爬 | L2 页面解析 + L3 DDG 兜底 |
| SVG favicon / 非标准路径 | L2 解析 `<link>` |
| 隐私（外部请求） | 直连同源为主；L3 DDG 加选项说明 |
...
3. **L3（DDG）作为可选增强**——需选项开关 + 隐私说明，评估后决定是否默认。
