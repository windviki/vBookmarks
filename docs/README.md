...
- **Deep scroll positions survive slow machines** (#68): with the highlight off and the tree scrolled a couple of pages down, the popup could reopen at the wrong spot — the restore silently deadlocked against the tree's own layout laziness (off-screen rows only lay out once the viewport passes over them), so the stored position never became reachable and the restore gave up. The restore now detects the stall and walks the tree band by band from the top — forcing each band to lay out — until the exact position lands.
...
- The lazy-layout deadlock is reproduced deterministically by a real-popup probe (`diag-68-slow-settle-duel.js`, a freeze-style slow-settle simulation): stuck at the clamp frontier before the fix, exact landing after two walk passes after it. The round is archived under `docs/issues/issues-65-66-memory-scroll.md`.
