...
- **Deep scroll positions survive slow machines** (#66–#68): with the highlight off and the tree scrolled a couple of pages down, the popup could reopen at the wrong spot — the restore silently deadlocked against the tree's own layout laziness (off-screen rows only lay out once the viewport passes over them), so the stored position never became reachable and the restore gave up. The restore now detects the stall and walks the tree band by band from the top — forcing each band to lay out — until the exact position lands. A field report on the same build sharpened the picture for long folders with the highlight on: the walk could land mid-settle and starve on the retry budget (giving up partway), and Chromium's scroll anchoring could then drag the landed view away as late content growth kept arriving. The walk now runs on its own budget scaled to the depth, scroll anchoring is suppressed for exactly the duration of the restore campaign, and the campaign only ends after the geometry has been stable for several checks — so the anchoring resumes protecting a final view instead of hijacking a settling one. The restore's own give-up can no longer corrupt the saved position either (its trailing scroll event used to overwrite it).
