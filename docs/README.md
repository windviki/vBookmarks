...
- Unified locale tooling (`scripts/i18n.py`): audit, missing-key reports, LLM batch translation, verify gate. Baseline grew from 75 to **345 keys** at 4.0 (**685** as of 4.1.0), all 43 locales aligned.
...
#   scripts/screenshots/shots-store.js   WebStore candidates: strip/promo×3/themes/options (7 views; pick 5)
...
# Release build & store zip (version read from manifest.json; esbuild bundle + Terser into dist/)
...
- **Deep scroll positions survive slow machines** (#66–#68): with the highlight off and the tree scrolled a couple of pages down, the popup could reopen at the wrong spot — the restore silently deadlocked against the tree's own layout laziness (off-screen rows only lay out once the viewport passes over them), so the stored position never became reachable and the restore gave up. The restore now detects the stall and walks the tree band by band from the top — forcing each band to lay out — until the exact position lands. Two field reports on the same build sharpened the picture further. First, for long folders with the highlight on: the walk could land mid-settle and starve on the retry budget (giving up partway), and Chromium's scroll anchoring could then drag the landed view away as late content growth kept arriving. The walk now runs on its own budget scaled to the depth, scroll anchoring is suppressed for exactly the duration of the restore campaign, and the campaign only ends after the geometry has been stable for several checks — the anchoring resumes protecting a final view instead of hijacking a settling one. Second, the position itself is now remembered **by row, not by pixel**: late correction waves trigger content-stable anchoring compensation — the view looks identical while the underlying pixel quietly changes — so a pixel memory random-walked across sessions and every reopen landed somewhere slightly different. The remembered row at the top of the viewport is persisted alongside the pixel and the restore lands on the row, immune to per-session layout differences. The restore's own give-up can no longer corrupt the saved position either (its trailing scroll event used to overwrite it).
- **The highlight no longer drags the reopened view around** (#66, #67): with "Highlight the last opened bookmark" on, reopening the popup after scrolling deep could shift the view toward the highlighted row. The cause was Chromium's scroll anchoring preferring the focused row as its anchor during the post-render layout settle; the highlight's focus grant now waits until the restored position has landed, so the anchoring protects the view instead of hijacking it.
- **The saved position can no longer be corrupted mid-restore** (#66, #68): on slower machines with big trees, the restore's own climb intermediates could be written back over the saved position (a popup closed mid-climb then "forgot" where you really were), and the retry budget was too short for slow settles. The retry now runs a ~4.5s campaign that never persists its own intermediates — your real scrolls take over instantly — and ends quietly at the best reachable position if the tree can never hold the target.

#### Engineering

- The lazy-layout deadlock is reproduced deterministically by real-popup probes: `diag-68-slow-settle-duel.js` (a freeze-style slow-settle simulation — stuck at the clamp frontier before the walk, exact landing after) and `diag-413-highlight-drift.js` (the maintainer's live repro — long folder, highlight on, wrapped titles, plus a modeled headed-machine placeholder phase: landed-then-anchored-away to 57% past the target before the fix, pixel- and content-exact after). The rounds are archived under `docs/issues/issues-65-66-memory-scroll.md`.
...
- **A layered Memory options group** (#65): "remember previous state" is no longer all-or-nothing. A master *Remember previous state* switch now heads a dedicated Memory group with four independent sub-toggles — *Highlight the last opened bookmark* (the most-requested off-switch, on by default), *Remember the scroll position*, *Remember opened folders*, and *Remember the last search* — while *Remember the last active view* keeps its own semantics beside them. Defaults change nothing; turning one layer off leaves the others intact, in the tree and the list views alike.
- **A multi-style Custom CSS workbench**: custom CSS moved out of the cramped options textarea into its own editor page — one tab per style with its own name, description and enable switch, ◀/▶ buttons that reorder the cascade (later styles override earlier ones), and a full-width editor that scrolls long stylesheets internally. The concatenated enabled styles keep feeding the very same apply pipeline, and the old single-stylesheet setting migrates in as one entry automatically.
...
- **The last-bookmark highlight no longer fades away intermittently**: opening a bookmark and closing the popup could kill the final "where I was" write (the debounced store write raced the popup teardown, the same close-path class as the scroll fix above) — the highlight now survives every close via a synchronous shadow, and the same guarantee covers the remembered row in list views.
- **Staging selection mode on wide popups and the side panel**: the dashed group connector's horizontal tick painted ~27px to the right of its trunk, floating inside the title text (a wide-grid-only double offset), and the group-head glyph well sat half a pixel off the trunk axis — both re-anchored to the icon-axis law.
- **The Custom CSS editor geometry**: a long stylesheet now scrolls inside the editor instead of growing past it and overlapping the page footer, and the editor follows the browser window width (a leftover 560px ceiling from the retired inline editor used to pin it).
...
- The memory switches are pinned by a combination test matrix (master × every sub-layer × the view switch, both restore polarities, unit and real-browser E2E), and six locale translations of the new strings were corrected against their baselines; pre-4.0 release notes moved from the README into per-version files under `docs/changelog/`.
...
- **Search history, on your terms**: the record-search-history switch moves into the options Search group and now only toggles the area — recorded queries survive, re-enabling brings them back — the area gains an inline close × (the classic-experience preset closes it too), and a new *recent searches to show* count (3/5/10/20, default 5) crops the area to taste. The count is a synced preference like the recent-bookmarks count.
...
### Earlier versions (pre-4.0)

Each release note lives in its own file under [`docs/changelog/`](changelog/) (Chinese: the `.zh.md` twin). Newer entries may migrate into that table over time — the files are the permalink home for shipped release notes.

| Version | Date | Changelog |
| --- | --- | --- |
| v3.7 | 2026-05-10 | [Changelog](changelog/v3.7.md) |
| v3.6 | 2024-01-08 | [Changelog](changelog/v3.6.md) |
| v3.5 | 2023-09-04 | [Changelog](changelog/v3.5.md) |
| v3.4 | 2023-02-14 | [Changelog](changelog/v3.4.md) |
| v3.3 | 2023-02-02 | [Changelog](changelog/v3.3.md) |
| v3.2 | 2020-09-12 | [Changelog](changelog/v3.2.md) |
| v3.1 | 2020-07-03 | [Changelog](changelog/v3.1.md) |
| v3.0 | 2019-08-22 | [Changelog](changelog/v3.0.md) |
| v2.9 | 2019-08-22 | [Changelog](changelog/v2.9.md) |
| v2.8 | 2019-05-06 | [Changelog](changelog/v2.8.md) |
| v2.6 | 2013-10-21 | [Changelog](changelog/v2.6.md) |
| v2.5 | 2013-08-30 | [Changelog](changelog/v2.5.md) |
| v2.4 | 2013-08-29 | [Changelog](changelog/v2.4.md) |
| v2.3 | 2013-04-09 | [Changelog](changelog/v2.3.md) |
| v2.2 | 2013-04-02 | [Changelog](changelog/v2.2.md) |
| v2.1 | 2012-12-12 | [Changelog](changelog/v2.1.md) |
| v2.0 | 2012-11-01 | [Changelog](changelog/v2.0.md) |
| v1.9 | 2012-08-19 | [Changelog](changelog/v1.9.md) |
| v1.8 | 2012-08-01 | [Changelog](changelog/v1.8.md) |
| v1.7 | 2012-06-26 | [Changelog](changelog/v1.7.md) |
| v1.6 | 2012-06-24 | [Changelog](changelog/v1.6.md) |
| v1.5 | 2012-06-21 | [Changelog](changelog/v1.5.md) |
| v1.4 | 2012-06-20 | [Changelog](changelog/v1.4.md) |
| v1.3 | 2012-05-25 | [Changelog](changelog/v1.3.md) |
| v1.2 | 2011-11-30 | [Changelog](changelog/v1.2.md) |
| v1.1 | 2011-11-16 | [Changelog](changelog/v1.1.md) |
| v1.0 | 2011-11-15 | [Changelog](changelog/v1.0.md) |
