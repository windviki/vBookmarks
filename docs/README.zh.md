...
- **为 Chrome 未缓存的收藏网站抓取真实图标**：本版本会自动为浏览器没有缓存图标的收藏项抓取真实站点图标——先直连站点抓取，失败时走内置的第三方图标兜底服务商列表（favicon.run），每个服务商都有熔断与故障转移保护；当死链扫描代理会话存在时，图标发现会改经代理重试。加强后的图标缓存以动态字节预算为上限，压力下按半减淘汰。图标组新增控制项："抓取缺失的站点图标"（默认开）、"第三方图标兜底"以及"清空图标缓存"操作。
- **选项页重组为 15 个分区**：塞爆的 Views 区拆成 视图显示 / 图标 / 右键菜单 / 工具 / 统计 五个分区，每个区域都有呼吸空间。
- **存储占用横条**：图标组现在用横条展示图标、书签、设置与剩余空间各占存储配额多少（带字节格式的图例），随数据变化实时更新。
- **头部右上角显示 GitHub / Homepage 链接与版本号**。
- **备份可选择携带图标缓存**：新增"备份时包含图标缓存"选项，把加强后的按站点图标缓存一起打进设置导出/导入——关闭可保持备份精简，图标反正会自动重新抓取。
...
- **应用内公告横幅**：版本门横幅被基于拉取的"What's new"横幅取代（可通过新的"显示应用内公告"选项开关，内容从项目 GitHub 仓库拉取，至多每 6 小时一次）——v4.0.8 的横幅宣告本次图标加强版，并附 v4 指南与更新日志链接。
- **选项页样式打磨**：尾部提示不再与上方控件相隔过远，列表按钮与页面其他按钮共享统一的视觉重量。

#### 工程

- favicon 加强模块：发现链、内置服务商列表 + 逐服务商熔断/故障转移、缓存 + 队列。
