...
- **1229 个单元测试**，40 个 Vitest 套件覆盖全部模块——含钉住行对齐几何、z-index 层级表、各主题徽标对比度的契约测试。
- **Docker harness**：零控制台错误冒烟、真实浏览器键盘/视图验证套件（tab 条键盘模型、焦点区域、搜索双区、逐视图渲染——43 条硬断言），以及 5 主题 × 8 界面语言的截图套件（含 RTL 镜像断言）。
- 统一语言工具（`scripts/i18n.py`）：审计、缺失报告、LLM 批量翻译、verify 门禁。基准键从 75 增至 **282**，43 个语种全部对齐。
...
9. 可设置后台标签页打开书签。
10. 选项中可调节弹窗缩放级别。
11. **选项页**："自定义样式"分组自定义分隔线标题/URL/样式；"死链扫描"分组调节扫描并发与超时。
...
变更：仓库目录重组（`src/`、`pages/`、`css/`、`assets/`、`scripts/`）；删除过时 `release/` 与 MV2 遗留（留存 git 历史）；图标全量内联 SVG；语言基准增至 282 键，43 个语种经 `scripts/i18n.py` LLM 流水线重新对齐；测试增至 40 个文件 1229 例；Docker harness 扩展键盘/视图验证套件与多主题、多语言截图。
