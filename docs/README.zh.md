...
- **1262 个单元测试**，39 个 Vitest 套件覆盖全部模块——含钉住行对齐几何、z-index 层级表、各主题徽标对比度的契约测试。
- **Docker harness**：零控制台错误冒烟、真实浏览器键盘/视图验证套件（tab 条键盘模型、焦点区域、头部行方向链、各视图 ↑↓ 越顶含列表内工具行层级、横幅键盘可达、搜索双区、逐视图渲染——100 条硬断言），以及 5 主题 × 8 界面语言的截图套件（含 RTL 镜像断言）。
- 统一语言工具（`scripts/i18n.py`）：审计、缺失报告、LLM 批量翻译、verify 门禁。基准键从 75 增至 **284**，43 个语种全部对齐。
...
新增：死链扫描的第二通道支持**你自己的代理服务器**（http/https/socks5）——死链视图代理条上的一键添加按钮会校验地址、探测可达性（不可达直接拒收），保存后每次扫描生效；更换/移除同处可及，选项页同步展示并可清除。路由采用标记匹配的临时 PAC：只有扫描器自己的探测 URL 走代理，其他标签页不受影响，扫描结束/取消/弹窗关闭即还原（崩溃残留由 Service Worker 清扫）。结果更清晰：工具栏新增死链·受限计数，扫描出现直连失败且未配置代理时给出"配置代理以区分真死链与区域受限"的引导。

变更：`proxy` 权限改为安装时声明（Chrome 不允许将其列为可选权限）。该权限**仅在**配置了代理且扫描运行中（或添加流程的可达性探测时）被使用——不设置代理服务器、或不使用死链功能，任何代理代码路径都不会执行。
...
变更：仓库目录重组（`src/`、`pages/`、`css/`、`assets/`、`scripts/`）；删除过时 `release/` 与 MV2 遗留（留存 git 历史）；图标全量内联 SVG；语言基准增至 284 键，43 个语种经 `scripts/i18n.py` LLM 流水线重新对齐；测试增至 39 个文件 1262 例；Docker harness 扩展键盘/视图验证套件与多主题、多语言截图。
