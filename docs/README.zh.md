...
- 捐赠横幅重新设计为温和的三选项卡片（支持一下 / 稍后 / 不再显示）——不再有倒计时、焦点劫持和强制点击。
...
2. **完整键盘支持** —— ↑↓←→ 移动选择，Space/Enter 打开，Home/End、PageUp/PageDown、Delete 均符合直觉。
3. 选中书签/文件夹后按 `F2` 重命名。
4. 中键点击文件夹，在新标签组中打开其全部书签（自动配色）。
5. `Ctrl+F` 聚焦搜索框，`Esc` 清除搜索。
6. 拖拽排序；跨同步/本地存储的拖拽会被安全拦截并给出提示。
7. 可设置点击书签后是否关闭弹窗。
8. 可只显示书签栏内容（选项中开启）。
9. 可设置后台标签页打开书签。
10. 选项中可调节弹窗缩放级别。
11. **高级设置**（入口在设置页右上角）：自定义分隔线的标题/URL/样式。
...
新增：Ink 与 Paper 双 fable 主题；命令面板（`Ctrl/Cmd+K`）；快速收藏星标按钮；可折叠的最近添加区；捐赠卡片温和化重设计；同步状态呈现重做（安静圆点、本地化 tooltip、`（仅本地）`/`（已同步）`根标注、拖拽拦截 toast、"高亮未同步"淡显生效）。
