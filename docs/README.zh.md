...
13) 深色主题：浅色/深色/跟随系统三态，全量 design tokens

14) 可选的侧栏（Side Panel）模式：设置中开启后点击图标在侧栏打开（默认仍是弹窗），另有 Alt+Shift+B 快捷键

15) 模糊搜索：弹窗内 fzf 风格子序列匹配，标题命中高亮，支持中文

16) 最近添加书签分区：树顶部展示最近收藏的 20 条（可在设置中关闭）

17) 文件夹内容排序：右键菜单按标题/添加时间排序，支持子文件夹置顶与递归
