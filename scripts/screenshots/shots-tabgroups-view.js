...
// Shots land in /tmp/shots/tabgroups-view with the 34- series. 4.0.9 adds the
...
(async () => {
    ...
    // --- 4.0.9: window head row is the fold control + keyboard reachable -----
    ...
    // --- 4.0.9: the row icon columns line up with the group head's ------------
    ...
    // --- 4.0.9: selection mode keeps the same four columns -------------------
    ...
    // --- 4.0.9: the color connector-line style -------------------------------
    ...
    // --- 4.0.9: recently closed section + its own context menus --------------
    await page.evaluate(async () => {
    ...
    // …and the keyboard can drive it (the 4.0.9 focus bug: the menu took focus
    ...
    // --- 4.0.9: narrow / wide parity with the dead + dupes views -------------
