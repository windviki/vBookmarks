...
ENV_ARGS=()
for k in VBM_PERF_BOOKMARKS VBM_PERF_DUP_RATIO VBM_DUP_COPIES VBM_PERF_RUNS VBM_PERF_DUPES_RUNS VBM_PERF_SETTLE_MS VBM_DIAG_SKIP_TG VBM_DIAG_SKIP_DUPES VBM_DIAG_VIRTUAL VBM_DIAG_THROTTLE VBM_DIAG_ROWS VBM_DIAG_FREEZE VBM_TG_WINDOWS VBM_TG_GROUPS_PER_WIN VBM_TG_TABS_PER_GROUP VBM_TG_LOOSE; do
    if [[ -n "${!k:-}" ]]; then
        ENV_ARGS+=(-e "$k=${!k}")
    fi
done
