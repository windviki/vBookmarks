...
(async () => {
    ...
    // 2d2. 4.0.7 announce banner: the remote announcement layer renders a
    ...
    await page.evaluate(() => chrome.storage.local.set({
        ...
        vbmAnnounce: {
            ...
            data: { version: 1, messages: [{
                id: 'v407-whats-new', minVersion: '4.0.7', maxVersion: '4.0.8', channel: 'all',
                once: true, display: 'banner', kind: 'tip',
                titleKey: 'announceV407Title', textKey: 'announceV407Text',
                textFallback: { en: 'fully-featured dead-link view' },
                links: [
    ...
    const seen = await page.evaluate(() => new Promise(resolve =>
        chrome.storage.local.get('vbmAnnounceSeen', r => resolve(r.vbmAnnounceSeen || []))));
    if (!seen.includes('v407-whats-new'))
        errors.push(`announce dismiss not recorded: ${JSON.stringify(seen)}`);
    await page.evaluate(() => chrome.storage.local.remove(['vbmAnnounce', 'vbmAnnounceSeen', 'donationDisabled']));
    await page.reload({ waitUntil: 'networkidle0' });
    await sleep(900);
