...
| `src/palette.js` | ⌘K command palette (P2): overlay (`#command-palette`, markup lives in both popup pages) unifying fuzzy bookmark search, folder jump (`treeView.revealFolder`) and a command list (quick-add, new bookmark/folder/separator at `quickAddFolderId`). Toggled by Ctrl/Cmd+K (capture-phase, refused while dialogs are open) or globally via the `open-command-palette` command (background sets a `storage.session` `pendingPaletteOpen` flag then `chrome.action.openPopup`, falling back to a `?palette=1` popup window). Query synthesis: empty → commands only, `/`-prefixed → commands only (slash-command framework for later `/dupes` `/dead`), otherwise commands first + `VBMFuzzy.rank(...).slice(0, 50)`; index rebuilt from `getTree` on every open. `initPalette(ctx)` returns `{ open(), close(), isOpen() }`. No neatools inside. Unit-tested by `tests/palette.test.js` |
