...
| `src/palette.js` | ⌘K command palette (P2): overlay (`#command-palette`, markup lives in both popup pages) unifying fuzzy bookmark search, folder jump (`treeView.revealFolder`) and a command list (quick-add, new bookmark/folder/separator at `quickAddFolderId`, plus the `/dupes` cleaner). Toggled by Ctrl/Cmd+K (capture-phase, refused while dialogs are open) or globally via the `open-command-palette` command (background sets a `storage.session` `pendingPaletteOpen` flag then `chrome.action.openPopup`, falling back to a `?palette=1` popup window). Query synthesis: empty → commands only, `/`-prefixed → commands only (slash aliases prefix-match, e.g. `/d` → `/dupes`), otherwise commands first + `VBMFuzzy.rank(...).slice(0, 50)`; index rebuilt from `getTree` on every open. Executing `/dupes` switches to a duplicates mode: one row per group (oldest title + count + normalized URL) plus a clean-all row; each clean is ConfirmDialog-gated, removes serially (`bookmarks.remove`), then `onChanged` rebuilds the tree. `initPalette(ctx)` returns `{ open(), close(), isOpen() }`. No neatools inside. Unit-tested by `tests/palette.test.js` |
