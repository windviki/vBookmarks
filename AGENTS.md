...
| `src/sync-manager.js` | `SyncManager` class: bookmark sync-status cache (5 min TTL), bookmark event listeners, undo stack for deletions (max 10), configurable auto-refresh (default 60 s, min 20 s), dual-storage (`folderType`/`syncing`) support. Loaded only by `pages/popup.html` |
...
- **4-space indentation**, ES6+ in newer code (`const`/`let`, arrow functions, `async`/`await`); `src/neat.js`/`src/neatools.js` retain older prototype-based patterns — match the surrounding style of the file you edit.
- Page scripts are wrapped in an IIFE: `(window => { ... })(window)`; `src/background.js` uses `(() => { ... })()`.
...
- Use `$` (from `src/neatools.js`) for element lookup; UI labels are assigned in `initXxx()` functions on `DOMContentLoaded`, not in HTML.
...
- User-controlled text rendered into the omnibox must go through `xmlEncode` (`src/background.js`); HTML contexts use `htmlspecialchars` (`src/neatools.js`). Preserve these escapes when editing rendering code.
