...
| `src/sync-manager.js` | `SyncManager` class: bookmark sync-status cache (5 min TTL), bookmark event listeners, configurable auto-refresh (default 60 s, min 20 s), dual-storage (`folderType`/`syncing`) support. Loaded by `pages/popup.html` and `pages/sidepanel.html` |
